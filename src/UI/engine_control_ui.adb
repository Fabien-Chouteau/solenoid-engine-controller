...
--        This.Tabs.Set_Tab (2, "AUTO", This.Auto);
   ...
   begin
      ...
      then
         if This.Ignition.Value < PP_Range'First then
            Set_PP_Evt.Ignition := PP_Range'First;
         elsif This.Ignition.Value > PP_Range'Last then
            Set_PP_Evt.Ignition := PP_Range'Last;
         else
            Set_PP_Evt.Ignition := This.Ignition.Value;
         end if;

         if This.Duration.Value < PP_Range'First then
            Set_PP_Evt.Duration := PP_Range'First;
         elsif This.Duration.Value > PP_Range'Last then
            Set_PP_Evt.Duration := PP_Range'Last;
         else
            Set_PP_Evt.Duration := This.Duration.Value;
         end if;
         Giza.GUI.Emit (Set_PP_Evt'Access);
         Solenoid_Control.Set_Ignition (Set_PP_Evt.Ignition);
         Solenoid_Control.Set_Duration (Set_PP_Evt.Duration);
         return True;
      else
         return False;
      end if;
